...
export default function PhoneRevealSection() {
  ...
  const headlineMove = mapProgress(progress, 0.5, 0.68);
  const phoneEnter = mapProgress(progress, 0.72, 0.88);
  const backgroundColor = mixColor([255, 255, 255], [7, 23, 22], darkProgress);
  const headlineY = lerp(64, 0, headlineEnter);
  ...
  const phoneY = lerp(104, 0, phoneEnter);
  const phoneScale = lerp(0.94, 1, phoneEnter);
    ...
    >
      <div className="sticky top-0 h-screen overflow-hidden px-6 md:px-10">
        <div className="relative mx-auto h-full w-full max-w-[1180px]">
          ...
          >
            <h2 className="whitespace-nowrap text-[20px] font-semibold leading-[1.22] tracking-[0] text-white md:text-[28px] lg:text-[32px]">
              Ryff는, 찍어둔 영상을 릴스로 바꿔드려요.
            </h2>
          </div>

          <div className="absolute bottom-[-20px] left-1/2 -translate-x-1/2 md:bottom-auto md:left-auto md:right-10 md:top-[55%] md:-translate-y-1/2 md:translate-x-0">
            <div
              style={{
                opacity: phoneEnter,
